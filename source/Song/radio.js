...
var serverQueue = lista.queue.get(message.guild.id);
    ...
    if (args[0] == 'list') {
        ...
        let embedlist = new Discord.RichEmbed()
            ...
            .setColor('#36393e')
            .setFooter(`${prefixo}radio play [ID/NAME] - Start the Radio`, message.author.displayAvatarURL)
            .setThumbnail(scott.user.displayAvatarURL)
        var desc = "";
        ...
        join(radioJson, message, args, scott, prefixo);
    ...
    leave(message, serverQueue, scott);
    ...
    .setAuthor('Radio Commands', scott.user.displayAvatarURL)
        .setDescription(`**Status:** ${rad}`)
        .addField(`**${prefixo}radio [list]**`, `To know which radios I have.`)
        .addField(`**${prefixo}radio [play] [radio]**`, `To connect the radio to the server.`)
        .addField(`**${prefixo}radio [leave]**`, `To turn off the radio on the server.`)
            .setColor('#36393e')
...
async function join(radioJson, message, args, scott, prefixo) {
...
var permissions = voiceChannel.permissionsFor(message.client.user);
    ...
    if(controle == true) {
        ...
        message.channel.send({
            embed: {
                description: `<:okay:538503952900161538> - ${message.author}, **Connecting** the radio \`${radio.name}\` to the channel \`${message.member.voiceChannel.name}\``,
                color: 0x36393e,
            }
        }).then(raP => {
...
async function leave(message, serverQueue, scott) {
        ...
        if (!message.guild.members.get(scott.user.id).voiceChannel);
        else if (voiceChannel !== message.guild.members.get(scott.user.id).voiceChannel) return message.channel.send('<:314240199406387201:490756225575682049> | Você não está no canal que eu **estou** **\`conectado\`**');
        ...
        message.channel.send({
            embed: {
                description: `<:okay:538503952900161538> - ${message.author}, **turning off** the radio on the channel **\`${message.member.voiceChannel.name}\`**`,
                color: 0x36393e,
            }
        }).then(leaveR => {
